...
> **保存ボタンが押せないときはこれ。** 必須項目が2つあり、**片方はこの画面に無い**。
> 「アカウントを最初から作成」したサンドボックスは何もコピーされないので両方とも空になる。
> ビジネス名は別ページにあるためフォームがその場でエラーを出せず、**保存ボタンを
> 非アクティブにするしかない**。既定の見出しテンプレートが
> `{{YOUR_BUSINESS_NAME}} は Stripe と提携して…` なので、ビジネス名が無いと
> フォールバックすら解決できない。

**順番が大事。先にビジネス名を埋めること。**

1. **サンドボックスが選択されていることを確認**してから、
   **設定** → **ビジネス設定** → **公開情報**（`/test/settings/public`）を開く。
   **同じ URL が「今選ばれているアカウント」を編集する**ので、ここを間違えると本番を書き換える。
   - **公開ビジネス名** に `Realize Beauty` を入れて保存。サポートメールとサイト URL も埋めておく。
   - 保存できない場合は先に **設定** → **ビジネス設定** → **アカウントの詳細** で **国 = 日本** を設定する。
     空のサンドボックスは国も未設定のことがある。
2. ポータル設定に戻り、**始める方法** の **リンクを有効化** を押す。
3. **見出し** を入力する（例: `Realize Beauty のご請求内容の管理`）。
4. 下の表のとおりトグルを設定する。
5. **変更を保存**。まだ灰色なら、何かトグルを1つ動かして戻す（フォームの変更検知の切り分け）。

### 何を ON にするか

このアプリは**プラン変更・解約・再開を自前の API で実装している**（`SubscriptionService` の
`changePlan` / `cancel` / `resume`）。ポータルに委ねているのは支払い方法の管理だけ。

| 項目 | 設定 | 理由 |
|---|---|---|
| 請求書履歴の表示 | **ON** | 日本の小規模事業者は経理で請求書が要る |
| 支払い方法 | **ON** | ポータルの主目的。アプリ側に代替手段がない |
| 請求先情報（名前・住所・電話） | **ON** | — |
| 請求先情報の**メールアドレス** | **OFF** | Stripe 自身が「顧客の請求先メールアドレスをログイン認証に使わないでください」と警告。書き換えられると `salon_id` の対応付けが揺れる |
| サブスクリプションのキャンセル | **ON**（期間終了時）| 同期は確認済み。**キャンセル理由**も ON にすると離脱理由が取れる |
| プランの変更 | **OFF** | 技術的な問題ではない。アプリに自前の画面があり、デモで見せたいのは**アプリの中の**導線。同じ操作が2箇所にあると説明が散る。後から ON にするのは1クリック |
| 数量の更新 | **OFF** | サロン単位の課金でスタッフ数課金ではない |
| プロモーションコード | **OFF** | 今キャンペーンが無い |
| デフォルトのリダイレクトリンク | develop フロントの `/settings/plan` | 設定しておくと `billing_portal/sessions` に `return_url` を渡さなくてよくなる |

> **ポータル発の変更は正しく同期される**ことをコードで確認済み。`syncFromStripe` が
> `customer.subscription.updated` を受けて price ID からプランを引き直し、`cancel_at_period_end`
> も反映する。Webhook の順序保証が無いことまで考慮して `last_stripe_event_at` で古いイベントを捨てる。
>
> ただし `SubscriptionPlan::fromStripePriceId()` は**設定にない price ID に `null` を返し**、
> `array_filter` がそれを落とすため、**プランだけ古いまま status と期間が更新される**。
> 3つの Price ID を全部 Render に入れていれば起きないが、設定漏れがあると静かにずれる。

6. **保存して初めて既定の `bpc_...` 設定が生成され**、`/v1/billing_portal/sessions` が
   それにフォールバックできるようになる。ここが本体。
