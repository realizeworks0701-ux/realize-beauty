...
## サブスクリプション（ADR-029 でスコープ外にした項目）

> 実装済みの範囲は [ADR-029](../decisions/ADR-029-subscription-billing.md) / [docs/ui/settings-plan.md](../ui/settings-plan.md) を参照

- プラン内容の管理画面（現状はプラン・価格・機能の変更に config/billing.php の編集とデプロイが必要。plans テーブルを作らない判断とセットの積み残し）
- トライアル期間（Stripe 側で開始されたトライアルは status=trialing / trial_ends_at として受け取れるが、RB からトライアルを開始する導線は無い）
- 年額プラン（現状は月額のみ。Price を増やすだけでは済まず、プラン選択UIと日割り・切り替えの説明を作り直す必要がある）
- 請求書・支払い失敗メールの文面カスタマイズ（現状は Stripe の既定メールに委ね、RB からは送らない）
- Lite 向けにカルテ由来の指標でダッシュボードを再構成する検討（基本ダッシュボードは予約・売上を前提としており、予約機能を持たない Lite では空欄が目立つ）
- ダウングレード時の Google watch チャネル / LINE 連携の能動的な解除（現状は接続情報を保持したまま同期・通知だけを止める。再契約でそのまま復帰できる代わりに、対象外のサロン向けの外部通知を受け続ける）
- webhook 欠落時の定期リコンサイル（Stripe から契約を取り直すバッチ。現状は Stripe の再送と webhook イベントの failed 記録に依存しており、恒久的な取りこぼしは検知できない）
