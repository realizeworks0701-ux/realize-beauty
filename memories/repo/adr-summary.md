...
最終更新: 2026-09-07 ／ 対象: ADR-001〜029 および ADR-031（すべて Accepted。ADR-020 のみ ADR-027 により置換済み）
...
| 010 | Git Workflow | GitHub Flow ベース。**2026-09-07 に ADR-031 で `feature/* → develop → main` へ改定**（`develop` が develop 環境、`main` が本番へデプロイ）。ブランチ: feature/fix/docs/refactor/chore。コミット: feat/fix/docs/refactor/test/chore。main へ直接 Push 禁止、PR 必須。 |
...
| 022 | デプロイ構成 | フロント=Cloudflare Workers Static Assets（当初は Pages と記述。2026-07-22 `03b5820` で移行済みで、SPA フォールバックは `wrangler.jsonc` の `not_found_handling`）/ API=Render(Docker)+Managed PostgreSQL / 写真=R2。フロントは `VITE_API_BASE_URL` でAPI接続先を注入、CORSは `CORS_ALLOWED_ORIGINS`（未設定なら全拒否のフェイルクローズ）。`render.yaml`・`backend/Dockerfile` 一式。手順は docs/deployment.md。API は MVP 構成（artisan serve）。 |
...
| 031 | develop 環境と本番環境の分離 | `develop` ブランチ → develop 環境（デモ兼先行検証）、`main` → 本番の2環境構成。DB・R2・Stripe・LINE・Google・OpenAI・`APP_KEY` をすべて分離する。develop の `APP_ENV` は **`staging`** —— `StripeClient::assertModeMatchesEnvironment()` が production かどうかの二値判定であるため、これが Stripe をテストモードへ倒す唯一の手段（`STRIPE_ENFORCE_MODE` は無効化しない）。副作用で `ConfirmableTrait` が効かず `migrate:fresh` / `db:seed` が確認なしに走るため、`demo:reset` が**接続先のデータベース名**を検査して補う。DB は Neon 無料プラン（Singapore・`DB_URL`・期限なし・自動ウェイク）で、`entrypoint.sh` の `migrate` はコールドスタート用に最大5回リトライ。R2 は共用せず専用バケット（プレフィックス分離は未設定時 fail-open し、トークンはバケットスコープで権限の境界にならない）。本番も develop も `QUEUE_CONNECTION=deferred`（本番は長らく `sync` で、ジョブ例外が `DB::transaction` の外へ抜けて予約APIを 500 にしていたことが2026-09-07のダッシュボード実設定突き合わせで判明し、本ADRで両環境とも是正した）。develop の Render Web は `region: singapore`（本番の `oregon` とは意図的に不一致。日本の見込み客への体感速度を優先）。`render.yaml` は既存リソースの実プラン・実リージョン（本番: web `0.5c-512mb`、db `0.1c-256mb`/`oregon`/`diskSizeGB: 1`）を確認したうえで明示する（当初は`plan`を書かない方針だったが、Generate Blueprint 突き合わせ後に変更）。Stripe は named sandbox ＋ Webhook の `livemode` 検査、`StripeConfigException`→503 / `StripeApiException`→502 で日本語化。フロントは Worker を分け、本番の deploy は `wrangler deploy --env=""`。手順は docs/runbook-develop-env.md。 |
...
- **環境と資格情報の分離**（028, 029, 031）: DEV/develop と本番でキー・バケット・DB・`APP_KEY` を共有しない。取り違えは検査で落とす（`assertModeMatchesEnvironment()` / Webhook の `livemode` / `demo:reset` の接続先検査）。
...
- **ブランチ運用の統一**: ADR-010 の「GitHub Flow、`develop` なし」と、README.md / docs/standards/git.md の「`main` → `develop` → `feature/*`」が矛盾していた。ADR-031 で `feature/* → develop → main` に統一し、ADR-010 / CONTRIBUTING.md / README.md / docs/standards/git.md を揃えた（2026-09-07）。
